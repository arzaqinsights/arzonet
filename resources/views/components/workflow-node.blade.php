...
<div class="flex flex-col items-center" x-data="{ node: getWorkflowNode('{{ $nodeId }}') }" x-init="$watch('workflowNodes', val => node = getWorkflowNode('{{ $nodeId }}'))">
    <template x-if="node">
        <div class="flex flex-col items-center w-full">
            ...
            {{-- The Node Card --}}
            <div class="w-full max-w-sm bg-white border border-surface-200 rounded-lg p-5 shadow-sm relative space-y-3" :class="{'ring-2 ring-brand': activeNode === '{{ $nodeId }}'}">
                <div class="absolute -top-3 left-4 px-3 py-0.5 text-white text-[9px] font-black uppercase tracking-widest rounded-full shadow-sm"
                     :class="{
                         'bg-amber-500': node.type === 'wait',
                         'bg-blue-500': node.type === 'send_email',
                         'bg-emerald-500': node.type === 'add_tag' || node.type === 'remove_tag',
                         'bg-purple-500': node.type === 'if_else',
                         'bg-gray-700': ['create_deal', 'add_note', 'add_task', 'add_topic', 'remove_topic', 'transfer'].includes(node.type)
                     }">
                    <span x-text="getNodeTypeLabel(node.type)"></span>
                </div>
                
                <div class="flex justify-between items-start pt-2">
                    <div class="text-sm font-semibold text-surface-800" x-html="getNodeDescription(node)"></div>
                    <div class="flex gap-1 shrink-0">
                        <button type="button" @click.stop="editNode('{{ $nodeId }}')" class="p-1.5 text-surface-400 hover:text-brand hover:bg-brand/5 rounded transition-colors" title="Edit Step">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                        </button>
                        <button type="button" @click.stop="deleteNode('{{ $nodeId }}')" class="p-1.5 text-surface-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors" title="Remove Step">
            ...
            {{-- Branching --}}
            <template x-if="node.type === 'if_else'">
                <div class="flex w-full items-start justify-center gap-8 mt-2 relative">
                    <!-- True Branch -->
                    <div class="flex flex-col items-center flex-1 border-t-2 border-brand/20 pt-4 relative mt-4">
                        ...
                        <template x-if="node.next_true">
                            <div x-html="renderNode(node.next_true)"></div>
                        </template>
                        
                        <!-- Add Step Button for True Branch -->
                        <div class="pt-6 pb-2">
                    ...
                    <div class="flex flex-col items-center flex-1 border-t-2 border-brand/20 pt-4 relative mt-4">
                        ...
                        <!-- Recursive Component Call -->
                        <template x-if="node.next_false">
                            <div x-html="renderNode(node.next_false)"></div>
                        </template>
            ...
            <template x-if="node.type !== 'if_else'">
                <div class="flex flex-col items-center w-full">
                    <template x-if="node.next">
                        <div x-html="renderNode(node.next)"></div>
                    </template>
                    
                    <div class="pt-6 pb-2" x-show="!node.next">
