    ...
    <form @submit.prevent="saveSegment()">
        <div class="glass-card p-8 mb-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-black text-surface-900">Segment Details</h2>
                <form action="{{ route('admin.segments.destroy', $segment) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this segment?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-ghost text-red-500 hover:text-red-700">Delete Segment</button>
                </form>
            </div>
        ...
        <div class="flex items-center justify-between">
            ...
            <button type="button" @click="saveSegment" class="btn btn-primary" :disabled="rules.length === 0 || isSaving">
                <span x-show="!isSaving">Update Segment</span>
...
function segmentBuilder() {
    return {
        ...
        rules: @json($segment->rules ?? [['field' => '', 'operator' => '', 'value' => '']]),
        matchingCount: 0,
        ...
        init() {
            if (this.rules) {
                this.rules.forEach(rule => {
                    if (rule.operator === 'date_range' && rule.value) {
                        const parts = rule.value.split(',');
                        rule.value_start = parts[0] || '';
                        rule.value_end = parts[1] || '';
                    } else {
                        rule.value_start = '';
                        rule.value_end = '';
                    }
        ...
        saveSegment() {
            ...
            fetch('{{ route("admin.segments.update", $segment) }}', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
