...
@section('heading', 'Create Sales Sequence')
...
<div class="max-w-xl mx-auto animate-slide-up">
    ...
    <div class="glass-card rounded-md">
        <div class="p-6 border-b border-surface-100 bg-surface-50/50">
            <h3 class="text-sm font-black text-surface-900 uppercase tracking-tight">New Drip Sequence</h3>
            <p class="text-xs text-surface-500 mt-1">Provide a descriptive name for this automated drip sequence.</p>
        </div>
        
        <form action="{{ route('admin.sequences.store') }}" method="POST" class="p-6 space-y-4">
            @csrf
            <div class="space-y-2">
                <label class="text-xs font-black text-surface-450 uppercase tracking-widest">Sequence Name</label>
                <input type="text" name="name" required class="form-input rounded-md bg-surface-50 border-surface-200 py-3 text-sm font-semibold w-full focus:border-brand focus:ring-0" placeholder="e.g. 3-Part Lead Welcome Journey">
            </div>
