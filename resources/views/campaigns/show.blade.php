...
<div class="space-y-8 animate-slide-up" x-data="{ tab: 'analytics', showExportLogsModal: false, exportLogsFormat: 'xlsx', exportLogsFilename: 'campaign_{{ $campaign->id }}_logs' }">
    ...
    @if($campaign->status === 'paused' && $campaign->error_message && str_contains($campaign->error_message, 'Auto-paused'))
        ...
        @endphp
        <div class="bg-gradient-to-r from-rose-50 via-rose-50 to-amber-50 border border-rose-200 rounded-md overflow-hidden shadow-sm">
            <div class="px-5 py-4 flex items-start gap-4">
                {{-- Icon --}}
                <div class="w-10 h-10 rounded-full bg-rose-100 border border-rose-200 flex items-center justify-center shrink-0 mt-0.5">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-4 mt-3">
                        <div class="flex items-center gap-1.5 px-2.5 py-1 rounded bg-rose-100/80 border border-rose-200">
                            <span class="text-[9px] font-bold text-rose-500 uppercase tracking-wider">Bounce Rate</span>
                            <span class="text-xs font-black text-rose-700">{{ $currentBounceRate }}%</span>
                            <span class="text-[9px] text-rose-400">(limit: 15%)</span>
                        </div>
                        @if($currentComplaintRate > 0)
                        <div class="flex items-center gap-1.5 px-2.5 py-1 rounded bg-amber-100/80 border border-amber-200">
                            <span class="text-[9px] font-bold text-amber-600 uppercase tracking-wider">Complaint Rate</span>
                            <span class="text-xs font-black text-amber-700">{{ $currentComplaintRate }}%</span>
                            <span class="text-[9px] text-amber-500">(limit: 0.1%)</span>
                        </div>
                        @endif
                    </div>

                    {{-- Recommendations --}}
                    <div class="mt-3 pt-3 border-t border-rose-200/50">
                        <p class="text-[10px] font-bold text-rose-600 uppercase tracking-wider mb-1.5">Recommended Actions</p>
                        <ul class="text-[11px] text-rose-700/70 space-y-1 list-disc list-inside">
                            <li>Clean your email list by removing invalid, inactive, and hard-bounced addresses</li>
                            <li>Verify email addresses before importing using an email validation service</li>
                            <li>Consider creating a new campaign with a cleaned list instead of resuming this one</li>
                        </ul>
                    </div>
                </div>
            </div>
