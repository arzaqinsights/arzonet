        ...
        <div class="glass-card p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" class="form-input" placeholder="e.g. Rahul Sharma" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" class="form-input" placeholder="e.g. rahul@company.com" required>
                    @error('email')
                        <p class="text-red-500 text-[11px] mt-1 font-bold">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($group['permissions'] as $permKey => $perm)
                                <label class="flex items-start gap-3 p-3 border border-surface-150 rounded-sm hover:bg-surface-50 transition-all cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $permKey }}" @change="checkState()" class="perm-checkbox mt-0.5 rounded-sm border-gray-300 focus:ring-brand text-brand" @checked($user->hasPermission($permKey))>
