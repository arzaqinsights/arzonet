...
@section('heading', 'Edit Visual Workflow')
...
<div class="max-w-4xl mx-auto space-y-8 animate-slide-up pb-12">
    ...
    <form action="{{ route('admin.workflows.update', $workflow->id) }}" method="POST" id="workflow-form" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $workflow->name) }}" class="form-input rounded-md bg-surface-50 border-surface-200 py-3 text-sm font-semibold w-full focus:border-brand" required>
                    </div>
                    <div class="space-y-2">
                        <label class="text-xs font-black text-surface-400 uppercase tracking-widest">Description</label>
                        <input type="text" name="description" value="{{ old('description', $workflow->description) }}" class="form-input rounded-md bg-surface-50 border-surface-200 py-3 text-sm font-semibold w-full focus:border-brand">
                    </div>
                </div>

                <div class="flex items-center gap-3">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $workflow->is_active) ? 'checked' : '' }} class="w-4 h-4 text-brand border-surface-300 rounded focus:ring-brand">
...
<script>
    function workflowBuilder() {
        return {
            triggerType: '{{ old('trigger_type', $workflow->trigger_type) }}',
            triggerValue: '{{ old('trigger_value', $workflow->trigger_value) }}',
            ...
            init() {
                // For editing, load existing nodes.
                const oldNodes = @json(old('nodes', $workflow->nodes));
                if (oldNodes) {
                    try {
